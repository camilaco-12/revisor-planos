...
El esquema es el mismo del plugin revisor-planos (hidrosanitario), a proposito:
asi las dos tablas de hallazgos se pueden consolidar en una sola para la
coordinacion interdisciplinaria, sin transformar nada.
...
HOJA = "Hallazgos estructurales"
FILA_ENCABEZADO = 4
FILA_INICIO_DATOS = 5
DIR = os.path.dirname(os.path.abspath(__file__))
RAIZ_PLUGIN = os.environ.get("CLAUDE_PLUGIN_ROOT") or os.path.dirname(DIR)
PLANTILLA = os.path.join(RAIZ_PLUGIN, "templates", "hallazgos_estructural.xlsx")
...
def asegurar_destino(ruta_xlsx, dry_run=False):
    ...
    if not os.path.exists(PLANTILLA):
        raise ErrorEntrada(
            ...
            "Generala con:\n"
            f'  python "{os.path.join(DIR, "crear_plantilla.py")}"'
        )
...
def verificar_esquema(ws, ruta_xlsx):
    """Compara la fila 4 con el esquema esperado y aborta si no coincide.

    Sin esta comprobacion, un Excel con las columnas en otro orden aceptaria la
    escritura y dejaria cada dato en la columna equivocada, en silencio.
    """
    ...
    lineas = [
        "El Excel destino no usa el esquema de columnas de este plugin.",
        f"  Ruta: {os.path.abspath(ruta_xlsx)}",
        "",
        "Diferencias en la fila 4:",
    ] + diferencias + [
        "",
...
def validar_hallazgos(hallazgos):
    ...
    if errores:
        raise ErrorEntrada(
            ...
            "  Alta  = cota, nota o celda leida directamente en el plano\n"
            "  Media = evidencia parcial, o el dato se infiere del cuadro de materiales\n"
            "  Baja  = medicion a escala, plano rasterizado o detalle no entregado\n\n"
